...
# azonos, tesztelt implementációját a teljes indoklásért. Ennél a botnál a
...
THRESHOLD_SUGGESTION_FIELDS = [
    ("zone_width_pct", "numeric", "Zóna szélessége (%)"),
    ("touch_volume_ratio", "numeric", "Touch-volumen arány az átlaghoz"),
    ("htf_confluence", "bool", "Több idősíkú (4h) OB-egybeesés"),
]
...
async def run_once(state: dict, now: datetime) -> tuple:
    ...
    for c in to_send:
        ...
        zone_width_pct = (zone["top"] - zone["bot"]) / zone["bot"] * 100 if zone["bot"] > 0 else None
        register_signal_audit(state, symbol, direction, "OB_RETEST",
                                None, c["current_price"], now,
                                meta={
                                    "zone_width_pct": zone_width_pct,
                                    "touch_volume_ratio": zone.get("touch_volume_ratio"),
                                    "htf_confluence": zone.get("htf_confluence"),
                                })
